...
<p align="center"><sub>模拟效果图。第一行是同一份额度在官方版下的样子，下面是补丁后同一份额度在四档用量下的样子。</sub></p>
...
**我的 Codex 每周额度什么时候重置？**
打了补丁以后底栏直接告诉你：`weekly █░░░░░░░░░ 15% (resets in 18h 58m)`。没有补丁就得敲 `/status`，从卡片里读重置时间。每周额度是固定的 7 天窗口，到点整体清零，不是慢慢滚动恢复的。

**为什么 Codex 底栏额度快用完了颜色也不变？**
因为官方是按项目**类型**上色，不看数值——5h 和 weekly 不管数字多少都是同一个颜色，而 `status_line_use_colors` 只是个开关。没有任何设置能做阈值变色，这正是第一个补丁做的事。

**能不能用脚本自定义 Codex 底栏？**
不能。Claude Code 的 `statusLine` 可以挂自己的命令，Codex 只能在 `[tui] status_line` 里从内置项里挑，没有留任何脚本入口。所以这个项目只能改源码重新编译。

**额度数据是从哪来的？**
来自 Codex 每次回复时本来就会收到的额度快照，和 `/status` 显示的是同一份数字。不额外发任何请求，也没有数据离开你的电脑。

**为什么百分比从「还剩」变成了「已用」？**
官方显示余量（`weekly 85% left`），补丁显示用量（`weekly 15%`），这样数字和旁边进度条的方向一致，也和 `Context N% used` 一致。

**Linux 和 Windows 能用吗？**
补丁本身跨平台，只改 TUI 这一个 crate。但编译和安装脚本是按 macOS 的独立安装目录（`~/.codex/packages/standalone`）写的，别的平台需要自己写安装脚本。

**Codex 升级会不会把它弄没？**
会，而且没有任何提示。见 [Codex 升级之后](#codex-升级之后)。
